...
{
  // The drone is considered as "arrived on poseref if it in a radius of 0.25m"
           ...
           (lastPoseReceived.z - next_z) * (lastPoseReceived.z - next_z)) < 0.25)
  {
    ...
    // double labyrinth_x[] = {0, 3, 3, 0, 0};
    // double labyrinth_y[] = {0, 0, 1.5, 1.5, 0};

    // ligne droite
    // double labyrinth_y[] = {0, 0, 0, 0, 0, 0, 0, 0, 0};
    // double labyrinth_x[] = {0, -1, -2, -3, -4, -3, -2, -1, 0};
...
// In our final approach to explore the map, the pathplanning creates a grid. This grid is made of
...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    {
      ROS_INFO("I am starting the SUPER 8 mission");
      myPath.takeoff = true;
      myPath.SetRef(0.0, 0.0, 2.0, 0.0);
      myPath.publish_poseref();
      ros::Duration(15).sleep();
      myPath.SetRef(2.0, 0.0, 2.0, 0.0);
      myPath.publish_poseref();
      ros::Duration(35).sleep();
      myPath.SetRef(2.0, 2.0, 2.0, 0.0);
      myPath.publish_poseref();
      ros::Duration(35).sleep();
      myPath.SetRef(0.0, 0.0, 2.0, 0.0);
      myPath.publish_poseref();
      ...
      myPath.landing = true;
      myPath.publish_poseref();
      ROS_INFO("I have finished the SUPER 8 mission");
    }
